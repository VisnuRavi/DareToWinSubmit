...
TOKEN = '1179329463:AAGNDY6Yn0-HGrwy6izes4cUInkzStkVbTQ'
...
class Game(object): 
    def __init__(self):
        ...
        self.state = {
                ...
                #'dares_received': False, #No longer needed as /getdares removed
                'accept': False,
                'pass_dare': False,
                'doing_dare': False,
                'collect_votes': False,
...
def gamestart(update, context):
    ...
    if chat_id in games.keys() and games[chat_id].state['start_state'] and \
        ...
        if len(game.players) == 1:
            ...
            #context.bot.send_message(chat_id, game.players_yet_to_play)
            context.bot.send_message(chat_id, 
...
def next_turn(chat_id, context):
    ...
    next_player_name = game.players[next_player_id][0]
    ...
    context.bot.send_message(chat_id, 
                             ...
                             .format(next_player_name, assigned_dare, assigned_dare_points))
    ...
    try:
        if chat_id in games.keys() and games[chat_id].state['players_assembled'] and \
            ...
            if len(game.players) == len (game.dares):
                ...
                #games[chat_id].state['dares_received'] = True #No longer needed as /getdare removed
                context.bot.send_message(chat_id, 
                ...
                '''
                starting_player_id = game.players_yet_to_play[0]
                starting_player_name = game.players[starting_player_id][0]
                games[chat_id].current_player_id = starting_player_id
                games[chat_id].players_yet_to_play.remove(starting_player_id)
                dare_id = game.players[starting_player_id][2]
                assigned_dare = game.dares[dare_id][2]
                assigned_dare_points = game.dares[dare_id][1]
                games[chat_id].state['accept'] = True
                games[chat_id].state['pass_dare'] = True
                #games[chat_id].state['dares_received'] = False #No longer toggled as /getdare removed
                context.bot.send_message(chat_id, 
                                        "{}'s dare is {}, worth {} point(s), enter /accept\
                                        if you are DARING or /pass if you want to pass the dare\
                                        to its creator to do it!"
        ...
        update.message.reply_text("Please enter your dare in the following format:\
                                \n/dare 'number of points it's worth' 'your dare'\
                                \n and dare points must be more than 0.")
...
def get_dare(update, context):
    ...
    player_id = player.id
    chat_id = update.effective_chat.id
    if chat_id in games.keys() and games[chat_id].state['dares_received'] \
        and player_id in games[chat_id].players_yet_to_play:
        game = games[chat_id]
        games[chat_id].state['accept'] = True
        games[chat_id].state['pass_dare'] = True
        games[chat_id].state['dares_received'] = False #Toggled to ensure only this player plays the turn
        games[chat_id].current_player_id = player_id
        games[chat_id].players_yet_to_play.remove(player_id)
        dare_id = game.players[player_id][2]
        assigned_dare = game.dares[dare_id][2]
        assigned_dare_points = game.dares[dare_id][1]
        update.message.reply_text(player_name + "'s dare is " + assigned_dare + ', worth ' + 
                                  str(assigned_dare_points) + ' point(s), enter /accept if you accept'
...
def pass_my_dare(update, context):
    ...
    player_id = player.id
    chat_id = update.effective_chat.id
    if chat_id in games.keys() and games[chat_id].players[player_id][3]:
        game = games[chat_id]
        games[chat_id].players[player_id][3] = False #reset own dare flag

        games[chat_id].change_state_false(['doing_dare'])    
        #games[chat_id].state['dares_received'] = True #No longer toggled for next player's turn
        ...
        if not game.players_yet_to_play:
            ...
            games[chat_id].change_state_true(['end_game', 'next_round'])    
...
def done_dare(update, context):
    ...
    if chat_id in games.keys() and games[chat_id].state['doing_dare'] \
        ...
        games[chat_id].change_state_false(['doing_dare', 'pass_dare']) 
...
def main():
    ...
    #                      port=int(PORT),
    #                      url_path=TOKEN)
    #updater.bot.setWebhook('https://young-ridge-31610.herokuapp.com/' + TOKEN)
    updater.idle()
