    ...
    try:
        if chat_id in games.keys() and games[chat_id].state['players_assembled'] and \
            ...
            if len(game.players) == len (game.dares):
                ...
                games[chat_id].state['dares_received'] = True
                context.bot.send_message(chat_id, 
                                        "All dares have been placed, enter /getdare "
                                        "to get your dare!")
...
def pass_my_dare(update, context):
    ...
    player_id = player.id
    chat_id = update.effective_chat.id
    if chat_id in games.keys() and pass_dare and games[chat_id].players[player_id][3]:
        game = games[chat_id]
        games[chat_id].players[player_id][3] = False #reset own dare flag
        games[chat_id].state['dares_received'] = True #Toggled for next player's turn
        ...
        context.bot.send_message(chat_id, 
                                player_name + ' has chickened out of his/her dare and has lost 2 points, to'
