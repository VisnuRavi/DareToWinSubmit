...
def start(update, context):
    ...
    try:
        if start_state is False:
            ...
            players[player_id] = [player_name, 0, False, '', 0]
...
def join(update, context):
    if start_state is True:
        ...
        if player_id not in players.keys():
            players[player_id] = [player_name, 0, False, '', 0]#place dare
        ...
        if players_assembled is True and dares_received is False and player_id in players.keys():
            ...
            if len(players) == len(dares):   
                ...
                for dare_id,player_id in zip(dares_ids, players_ids):
                    ...
                    value_player[3] = value_dare[2]
                    value_player[4] = value_dare[1]
                dares_received = True
...
def get_dare(update, context):
    ...
    if dares_received is True and player_id in players.keys():
        player_name = update.effective_user.first_name
        assigned_dare = players[player_id][3]
        assigned_dare_points = players[player_id][4]
        doing_dare = True
        update.message.reply_text(player_name + ' dare is ' + assigned_dare + 'worth ' + 
                                  str(assigned_dare_points) + ', type /donedare after'
    ...
    if collect_votes:
        ...
        if player_id == current_player_id:
            ...
            if votes == len(players) - 1:
                ...
                if dare_num == len(players):#able to go to 2 functions depending on input
                    end_game = True
                    next_round = True                 
...
def next_round(update,context):
    ...
    if next_round:
        ...
        for key,value in players.items():
            players[key] = [value[0], value[1], False, '', 0]#to go round
